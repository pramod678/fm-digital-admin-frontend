...
const SelectFeatureArtist: React.FC<SelectProps> = ({ control, name, errors, required, id }) => {
    ...
    const artistSource: any[] = GetFeaturingArtist?.data?.data ?? [];

    const uniqueOptions = artistSource.filter(
        (option: any, index: number, self: any[]) =>
            self.findIndex(
                (o: any) =>
                    o.FeaturingArtist?.toLowerCase().trim() ===
                    option.FeaturingArtist?.toLowerCase().trim()
            ) === index
    );

    const typeaheadOptions = uniqueOptions.map((o: any) => ({
        value: o.FeaturingArtist,
        label: o.FeaturingArtist,
    }));
                                    ...
                                    {selectedArtists.map((artist) => (
                                        ...
                                        >
                                            ...
                                            <button
                                                ...
                                                className="ml-1 text-purple-500 hover:text-purple-800 font-bold leading-none focus:outline-none"
